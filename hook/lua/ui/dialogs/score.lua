...
function CreateSkirmishScreen(victory, showCampaign, operationVictoryTable)
    ...
    LayoutHelpers.AtCenterIn(bg, controls.mapGroup)

    bg.brackets = UIUtil.CreateDialogBrackets(bg, 40, 30, 40, 30)

    LayoutHelpers.FillParent(dialog, GetFrame(0))

    movieBG.Height:Set(controls.mapGroup.Height)
    movieBG.Width:Set(function()
        local ratio = controls.mapGroup.Height() / 1024
        return 1824 * ratio
    end)
    ...
    LayoutHelpers.AtCenterIn(movieBG, controls.mapGroup)
